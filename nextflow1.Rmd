...
This will create the `nextflow` executable that can be moved, for example, to `/usr/local/bin`.<br>
...
This command downloads and runs the pipeline `hello` (it is downloaded from the `nextflow-io` [github repository](https://github.com/nextflow-io)).
...
We can write a very simple Nextflow script: save the following piece of code in the `test.nf` file:
...
- Let's create a couple of files (e.g. paired-end reads) and let's try to read them as a [tuple](https://www.nextflow.io/docs/latest/process.html?highlight=tuple#input-of-type-tuple).
...
Let's add a process to the previous script `test.nf`:
...
This code indicates **the path** in which theprocess is "isolated" and where the corresponding temporary files are kept in the **work** directory. <br>
