...
- A queue channel is a non-blocking unidirectional [FIFO](https://en.wikipedia.org/wiki/FIFO_(computing_and_electronics)) (First In, First Out) queue which connects two processes or operators. 
...
An operator is a method that reshape or connect different channels applying some rules.
...
- Try to reshape the input channel using different operatoes by generating:
  - A single emission
...
Channel
    .fromPath( "*.txt").set {my_fyles}

my_fyles.collect().view()
my_fyles.combine(my_fyles).view()
my_fyles.collect().map{
	["id", it]
}.view()
my_fyles.view()
```
...
## EXERCISE 2
 - Change the pipeline for producing files instead of standard output. You can write another process to handle the fact that you have a list in the workflow2. You need also to specify within the workflow what to output using **emit** keyword.
...
- Change the pipeline to use only one process to handle both the cases (one element or a list), you can chose the elements from a list using the positional keys (i.e. list[0], list[1], etc...)
