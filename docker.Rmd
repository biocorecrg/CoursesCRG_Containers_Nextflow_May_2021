...
1. <u>`alpine` image.</u>
  1. **Search** and **pull** the `alpine` image (tag **3.12**) - it is an **official build**.
  2. Can you run a container from this image and make it print a “hello world” message?
  3. Now run a container **interactively** from the same image.
    * Run `whoami` in the container.
    * Exit the container and run `whoami` on the host machine: do you get the same output?
  4. Restart the container you just exited:
    * Is it now running?
    * Make the container execute the command `ls`.
    * Stop the container.
  5. Remove the alpine image and all its containers (running or stopped)
...
2. <u>`blast`</u> with volumes (ONGOING)
  1. Pull the `biocontainers/blast`, if you didn't do it earlier today.
2. Get the 2 following **fasta files** from uniprot (protein sequences) and save them in the newly created **blastp_test** folder:
  * https://www.uniprot.org/uniprot/O75976.fasta
* https://www.uniprot.org/uniprot/Q90240.fasta
3. Align those 2 sequences with `blastp`, with a `biocontainers/blast` container.
...
# Run the image with volumes
docker run -v /users/bi/sbonnin/test_docker/blastp_test:/blastdb biocontainers/blast:latest blastp -query /blastdb/O75976.fasta -subject /blastdb/Q90240.fasta
```
...
We can save this recipe in a file called **Dockerfile_ARG**.
<br>
  
  Build the image to get the default version of Python (version 2.7, as given by ARG in the recipe):
  
  ```
docker build -t Py2.7 -f Dockerfile_ARG .
```

Install version 3.8 of Python instead (give the argument via the `--build-arg` option):
...
2. <u>random numbers</u>
  1. Copy the following short `bash` script in a file called `random_numbers.bash`.
```{bash}
#!/usr/bin/bash
...
This script outputs **random intergers from 1 to 1000**: the number of integers selected is given as the **first argument**.
  2. Write a recipe for an image:
    * Based on **centos:7**
    * That will execute this script (with `bash`) when it is run, giving it **2** as a default argument (i.e. 2 random integers): the default can be changed as the image is run.
    * Build the image
    * Start a container with the default argument, then try it with some other numbers.
...
docker run -d -v $HOME/myscratch:/scratch -p 8083:3838 --name myfastqc fastqcwww
```
...
```{block2, type='rmdnote'}
NOTE about context here ... to see if this has been explained before
```
...
```{block2, type='rmdtip'}
Approaches to have the code or mounting.
```
