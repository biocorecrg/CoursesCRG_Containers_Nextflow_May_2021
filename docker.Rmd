...
* You can choose the version of Ubuntu you are fetching: check the different tags on the website:
...
Where is the image?
<br>
Run `docker images` in the terminal, to see a list of the most recently created images with information such as the **repository, the unique image ID, the tag and size**.
...
* How many images do you get if you run `docker images --all`? What are those?
...
Run container as daemon (in background) *explain why we would want to do this (run as daemon)!*
...
*Maybe it is interesting to have here docker rm?*
...
* `docker exec` needs an already running container to queried command.
...
Exercise on playing with run and exec?
