...
<br>
Default version should be 2.7, and try to pass the argument so that version 3.8 is installed.
<br> Build and run first with the default version, and then with version 3.8.
...
Recipe saved in **Dockerfile_ARG**
...
Build the image install version 3.8 of Python instead (via the `--build-arg` option):

```
docker build --build-arg PyVersion=3.8 -t py38 -f Dockerfile_ARG .
```

Run the image and check if the python of the correct version is installed:
