...
```{block2, type='rdmtip'}
In order to avoid that some directories or files are inspected or included (e.g, with **CP** command in Dockerfile), you can use .dockerignore file to specify which paths should be avoided. More information at: https://codefresh.io/docker-tutorial/not-ignore-dockerignore-2/
```
...
You can also use **ARG** to build a specific software version in the image:
...
Now run the image and check if the python of the correct version is installed:
...
A more complex recipe (save it in a text file named **Dockerfile_ubuntu**!):
...
Use `docker tag` in order to **tag a local image** that has - for example - ID "f9f41698e2f8" into the "ubuntu_wget" image name repository with version/tag "1.0":

```{bash}
docker tag f9f41698e2f8 --tag ubuntu_wget:1.0
```

If the version/tag is not specified, it defaults to **latest**.


### Build cache

Every line of a Dockerfile is actually an image/layer by itself (you can see all images with `docker images --all`.
<br>
Modify for instance the last bit of the previous image recipe (**Dockerfile_ubuntu**) (let's change the image URL) and rebuild it (even with a different name/tag):
...
```{block2, type='rmdnote'}
As commented in previous sections, the context is the place from where Docker will start the build process. Keep it in mind when defining the paths when using commands such as **CP** in the recipe.
```
